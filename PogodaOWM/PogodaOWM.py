...
# from qgis.core import QgsVectorLayer, QgsField, QgsMapLayerRegistry, QgsCoordinateReferenceSystem
...
class PogodaOWM:
    ...
    def run(self):
        ...
        if result:
			...
			#Dodaj podklad mapowy
			radio1=self.dlg.radioButton.isChecked()
			radio2=self.dlg.radioButton_2.isChecked()
			radio3=self.dlg.radioButton_3.isChecked()
			if radio1:
				uri = 'crs=EPSG:4326&dpiMode=7&featureCount=10&format=image/png&layers=Raster&styles=&url=http://mapy.geoportal.gov.pl/wss/service/img/guest/ORTO/MapServer/WMSServer'
				rlayer = iface.addRasterLayer(uri, 'Ortofotomapa', 'wms')
			elif radio2:
				uri = 'crs=EPSG:4326&featureCount=10&format=image/png&layers=Administracyjna&styles=&url=http://mapy.geoportal.gov.pl/wss/service/img/guest/Administracyjna/MapServer/WMSServer'
				rlayer = iface.addRasterLayer(uri, 'Mapa Administracyjna', 'wms')
			elif radio3:
				sc=os.path.join(os.path.expanduser("~"), ":/plugins/PogodaOWM/POL_adm1.shp")
				uri = QgsVectorLayer(sc, 'Granice wojewodztw', 'ogr')
				if not uri.isValid():
			...
			check1=self.dlg.checkBox.isChecked()
			if check1:
				palyr = QgsPalLayerSettings() 
				palyr.readFromLayer(warstwa) 
				palyr.enabled = True 
				palyr.fieldName = 'Miasto' 
				palyr.placement= QgsPalLayerSettings.Line 
				# palyr.placement= QgsPalLayerSettings.OverPoint 
				# palyr.quadrantposition= QgsPalLayerSettings.QuadrantOver
				palyr.setDataDefinedProperty(QgsPalLayerSettings.Size,True,True,'12','') 
				palyr.writeToLayer(warstwa) 
			...
			t=iface.mapCanvas().setMapTool(tool)
			check2=self.dlg.checkBox_2.isChecked()
			if check2:
				layer = iface.mapCanvas().currentLayer()
				registry = QgsSymbolLayerV2Registry.instance()
				markerMeta = registry.symbolLayerMetadata("SimpleMarker")
				symbol = QgsSymbolV2.defaultSymbol(layer.geometryType())
				markerLayer=markerMeta.createSymbolLayer({'name': 'regular_star', 'color': '203,198,36', 'color_border': '255,0,0', 'offset': '0,0', 'size': '4', 'angle': '0'})
